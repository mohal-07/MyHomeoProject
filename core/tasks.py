...
def get_gemini_response(prompt):
    ...
    if not IS_GENAI_CONFIGURED:
        print("Error: Cannot call Gemini, API Key not configured or configuration failed.")
        return None
    try:
...
def _build_patient_data_json(case_instance):
    ...
    personal_hist = case_instance.personal_history if isinstance(case_instance.personal_history, dict) else {}
    menstrual_hist = case_instance.menstrual_history if isinstance(case_instance.menstrual_history, dict) else {}
    physical_exam = case_instance.physical_examination if isinstance(case_instance.physical_examination, dict) else {}
    menopause_data = menstrual_hist.get("menopause", {}) if isinstance(menstrual_hist.get("menopause"), dict) else {}
...
def process_case_analysis(case_id):
    ...
    try:
        case_instance = Case.objects.select_related('user').prefetch_related('past_illnesses').get(id=case_id)
        ...
        **METHODOLOGY:**
        ...
        8.  **Allopathic Suggestion (Optional):** If the clinical diagnosis suggests a condition where conventional treatment might be considered or necessary alongside homeopathy (e.g., for severe infections, emergencies, patient preference, co-management), provide a *brief, standard* allopathic prescription suggestion (Drug name, Dose, Route, Frequency, Duration). **Only provide this if clearly warranted by the clinical picture; otherwise, state "N/A" or omit the key.** Include brief justification or standard indications.
        ...
        **REQUIRED OUTPUT:**
        ...
        // ... list top 5-10 remedies from repertorization ...
        ],
        ...
        "allopathic_prescription_suggestion": {{ // Can be null or omitted if not applicable
        ...
        "justification_or_notes": "Brief reason or standard practice note (e.g., 'Standard first-line antibiotic for suspected strep throat', 'For breakthrough pain relief'). If not applicable, this whole object should be omitted or set to null/empty string."
        }}
        }}
        ...
        **IMPORTANT:** Ensure the entire output is a single, valid JSON object starting with {{ and ending with }}. Do not include any introductory text, explanations, apologies, or concluding remarks outside of the specified JSON structure. Focus solely on providing the requested analysis within the defined format."""
        ...
        if not response_data:
            print(f"Error: No response text received from Gemini for Case ID {case_id}.")
            raise ValueError("No response text received from AI.")

        print(f"Received response (length: {len(response_data)}) for Case ID: {case_id}. Stripping markdown...")
        # Try to extract JSON from markdown code blocks
        match = re.search(r"```(?:json)?\s*(.*?)\s*```", response_data, re.DOTALL | re.IGNORECASE)
        if match:
            json_content = match.group(1).strip()
            print(f"Extracted JSON content using markdown fences for Case ID: {case_id}.")
        else:
            print(f"Warning: JSON markdown fences not found for Case ID {case_id}. Attempting to use raw response.")
            ...
            if not (json_content.startswith('{') and json_content.endswith('}')):
                print(f"Error: Raw response doesn't look like JSON object for Case ID {case_id}.")
                raise ValueError("Response content does not appear to be a valid JSON object.")
            else:
        ...
        while os.path.exists(output_path):
            ...
            if counter > 100: # Prevent infinite loop
                raise IOError("Failed to determine unique output filename after multiple attempts.")
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            try:
                ...
                print(f"Error: Content validation failed. Invalid JSON for Case ID {case_id}. Error: {json_err}")
                raise ValueError("Generated content is not valid JSON.") from json_err

        print(f"Successfully saved result file for Case ID {case_id} at {filename_base}")
        ...
        try:
            if case_instance and case_instance.pk: # Check if we successfully fetched the instance earlier
                ...
                try:
                    ...
                    if current_case.status != final_status or (final_status == 'COMPLETED' and current_case.result_file_path != saved_file_rel_path):
                        ...
                        current_case.save(update_fields=['status', 'result_file_path', 'updated_at'])
                        print(f"Final update for Case {case_id}. Status: {final_status}, File: {saved_file_rel_path or 'None'}.")
                    else:
                        print(f"No final status update needed for Case {case_id}. Status already '{current_case.status}'.")
                except Case.DoesNotExist:
                     print(f"Error: Case {case_id} disappeared before final status update.")
                     # Cannot update status if it's gone

            elif case_id: # If case_instance was None (e.g., initial fetch failed), try to fetch again
                print(f"Attempting final status update for Case ID {case_id} even if instance fetch failed earlier.")
                try:
                    case_to_update = Case.objects.get(id=case_id)
                    if case_to_update.status != final_status: # Avoid unnecessary DB write
                         ...
                         print(f"Final status update for Case {case_id} set to {final_status}.")
                    else:
                         print(f"No final status update needed for Case {case_id}. Status already '{case_to_update.status}'.")
                except Case.DoesNotExist:
                    print(f"Error: Case {case_id} not found during final status update.")
                except Exception as final_update_e:
                    print(f"Error during final status update attempt for Case {case_id}: {final_update_e}")

        except Exception as final_save_e:
            print(f"Critical Error during final update block for Case {case_id}: {final_save_e}")
            traceback.print_exc()